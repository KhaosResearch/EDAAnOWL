...
AgoraOWL v0.8.0 includes a complete example (`examples/cred-asset-example.ttl`) demonstrating how to annotate a data space asset following [CRED recommendations](https://espaciosdedatos.gob.es/), covering all 9 entity types:
...
5. **DataProfile** — AgoraOWL semantic profile with metrics
...
> [!TIP]
> The CRED example demonstrates how AgoraOWL's unique DataProfile semantics (metrics, observable properties, CRS) complement the standard DCAT-AP cataloguing metadata.
...
AgoraOWL **does not bundle local SKOS vocabularies** for domain concepts. Instead, we recommend using **established external vocabularies** directly:
...
> [!IMPORTANT]
> **Why this approach?** AgoraOWL provides _structure_ (classes, properties) for semantic interoperability. Domain _content_ (concepts, terms) should come from globally recognized vocabularies to maximize cross-domain compatibility.
