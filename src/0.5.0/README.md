...
Starting with v0.5.0, AgoraOWL **no longer bundles local SKOS vocabularies** for domain concepts, observable properties, metrics, or sectors. Instead, we recommend using **established external vocabularies** directly:
...
Only `datatype-scheme.ttl` is retained, as it defines AgoraOWL-specific data classes (`:georaster`, `:tabular`, `:timeseries`) that don't have direct external equivalents.
