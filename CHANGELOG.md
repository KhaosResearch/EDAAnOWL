...
- **Semantic Units (QUDT)**: Added `edaan:hasMetricStandard` to link metrics to standardized units of measure (QUDT) or categorical standards (SKOS). (resolves #35)
- **Operational Meaning**: Added `edaan:measuresProperty` to explicitly link metrics to the `ObservableProperty` they quantify.
- **SIEX Alignment**: Rebranded SIEX vocabularies to the official `w3id.org/EDAAnOWL/siex/` namespace.
- **Validation**: Updated SHACL shapes and consistency tests for new v0.7.0 semantic features.

### Deprecated

- `:metricUnit` (string) in favor of `:hasMetricStandard` (ObjectProperty).
